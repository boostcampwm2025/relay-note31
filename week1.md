...
{
    // 1단계: 일상의 시적 발견
    const mood = readlineSync.question('지금 당신의 기분이나 마음에 떠오른 단어를 자유롭게 말해주세요: ');
    ...
    // 4단계: 시적 공동체 형성
    console.log('생성된 시와 감상을 슬랙 채널에 공유하여 동료들과 함께 시적 감성을 나누어보세요!');
}
```
...
showGrowthStats()
{
    ...
    console.log(`지속성: ${consistency}% (${this.profile.totalPoems}/${daysActive}일)`);
}
```
