...
챌린지 2주차의 간단한 일기를 밈과 연결해 보았다. 

각각 미션 6, 7, 8, 9를 수행하면서 경험하고 힘들었던 내용을 위주로 선택해 프롬프팅했다. 

```
다음과 같은 4가지 이야기를 한 그림에 담아줘: 
한쪽에서는 타노스가 움직이면서 나를 피하고 있어. 첨부한 1번 이미지를 참고해서 그려줘
다른 한쪽에서는 파일에 뒤덮인 나를 그려줘. 
다른 한쪽에서는 반복문을 사용하다가 다시 재귀를 사용하다가 반복문을 사용하는 나를 그려줘. 첨부한 2번 이미지를 참고해줘. 
또 다른 한쪽에서는 두 개의 프로세스가 달리기 경주를 하고 있고, 내가 그 모습을 고통스러워하면서 쳐다보는 이미지를 그려줘. 
```

**Gemini**

- 참고할 이미지 첨부 시 따로 주의사항이나 규약을 요청하지는 않았다.
- 이미지 생성을 요청했더니 이미지 생성 불가능 오류가 발생했고, 요청한 프롬프팅을 잘 이해하지 못한 것 같았다.
- 이미지뿐만 아니라 미션 수행 중간에도 프롬프팅을 이해하지 못하는 경우가 있었다. 전반적인 응답은 GPT보다 좋지만 가끔 오류가 나는 것 같다.

**ChatGPT**

- Gemini처럼 참고할 이미지 첨부 시 따로 주의사항이나 규약을 요청하지는 않았다.
- 프롬프팅을 요청했더니 1~2분 정도의 시간 후 이미지를 생성해줬다.
- 처음 나온 이미지가 마음에 들지 않아서, 부분적인 수정을 요청했더니 잘 알아듣지 못하는 것 같았다. 수정을 요청하지 않은 부분까지 수정하는 경우가 있었다.
    
    <img width="1024" height="1024" alt="Image" src="https://github.com/user-attachments/assets/1c2df3ae-b726-4ca4-9abf-712d904a8a5e" />
    

**Sora**

- 이미지를 첨부하려고 했더니 미디어 규약 동의를 요구했다. 미디어가 안전한지, 미디어의 권리를 가지는지 물어보고, 만약 문제가 생긴다면 사용 제재를 하겠다는 내용이 있었다.
>>>>>>> 8bb662637539cd17c0fe9fe72b7830d055b6c970
