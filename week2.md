...
슬랙 봇에 캠프토닥이에게 위로의 한마디를 들으며 힘든 하루를 잘 마무리할 수 있었습니다. 저는 DM으로 혼자 수신받았지만, 다른 분들이 있는 단체 채널에 사용한다면 함께 따뜻한 하루를 보낼 수 있을 것 같습니다.

현재 토닥 봇에 오류가 있어서 week1.md를 참고해서 봇을 수정해주세요..! 수정 전에는 프롬프트를 만들어 미션을 수행해도 좋을 것 같습니다🥰

| 11일차 | 12일차 | 13일차 | 14일차 |
|--------|--------|--------|--------|
| 객체지향 설계, 병렬처리, 비동기, EventEmitter, Promise. 데이터흐름, 비동기, 비동기 이벤트 | 단위 테스트, 스레드, 이벤트 루퍼 | file, git, git object, hash | git, git clone, git object |
| EventEmitter처럼 너는 분명 신호를 보내고 있어. 다만 지금은 응답받을 핸들러가 아직 준비되지 않았을 뿐이야. Promise처럼, 지금의 시간들은 pending 상태야. 하지만, 그건 실패가 아니라 완료를 향한 기다림이라는 뜻이야. | 에러 로그에 치이면서도 8시간을 채웠다는 건, 스레드처럼 끊기지 않고, 이벤트 루퍼처럼 다시 돌아와서 결국 계속 돌리고 있다는 거야. 오늘의 고군분투는 단지 ‘실패한 테스트’가 아니라 너 자신을 검증해나가는 과정 그 자체야. | 파일, 오브젝트, 해시... 눈앞에 흐릿해 보여도, 지금 겪는 이 낯섦은 표면을 넘어서 본질을 이해하려는 신호야. Git처럼 복잡하지만 논리적인 구조 안에 들어서고 있다는 뜻이니까. 지금 이 복잡함은 나중에, 누군가에게 “그거, 내가 깊게 파봤는데 말이야”라고 말할 수 있는 날을 위한 준비야. | Git의 오브젝트처럼, 지금은 보이지 않아도 하나하나가 쌓이고 있어. 지금 이 순간조차도 나중에 log로 돌아볼 수 있는 값진 한 줄이 될 거야. 오늘을 기록하고 내일을 준비하는 당신에게 박수를 보낼게. |
...
시를 지은 결과, 그날 하루에 한 일, 공부한 내용을 간단하게 시로 요약하는 일이 되었다. 

### 소감
ChatGPT에게 주제를 주고 시를 지어달라고 하면, 만족스럽지 않은 시가 나온다. 시에서 전하고 싶은 의도가 드러나지 않아서 만족스럽지 않은 것 같았다. 이후 해당 시를 바탕으로 직접 창작하려고 노력했다. 나중에 작성한 시를 생성형 AI에게 평가를 받고 개선, 수정하는 것도 좋을 것 같단 생각이 들었다. 

### 차후에 수정할 것 
다음부턴, 프롬프트를 좀 더 자세히 작성해서 내가 원하는 시에 가까운 결과물을 뽑아내는 걸 목표로 잡고 싶다. 그리고 하루 요약에 대해 많은 공감을 받았고, 하이큐가 아닌 다른 형태의 시나, 좀더 감정이 들어간 시를 작성하는게 좋다는 인상이 있었다. 
...
이번 주에는 GPT를 활용해 ‘수면 부족’을 주제로 네컷 만화를 제작하는 퀘스트를 진행했다.
늦은 시간까지 미션을 하며 커피로 버티다가, 결국 아침을 맞이하는 모습을 컷마다 담았다.



#### 깨달은 점

* AI를 활용하면 아이디어를 빠르게 구체화하고 시각 자료로 옮기는 속도가 훨씬 빨라진다.
* 구상과 제작을 번갈아 진행하며 콘셉트를 수정·보완하는 과정이 효율적이었다.
* AI가 제안하는 시각적 요소 중 일부는 생각하지 못했던 부분이라 창작의 폭이 넓어졌다.



#### AI 활용의 장점

1. **속도** – 구상부터 시안까지 혼자 하는 것보다 훨씬 빠름.
2. **다양성** – 여러 버전을 쉽게 시도하고 비교 가능.
3. **발상 확장** – 예상치 못한 디테일 제안으로 완성도를 높임.

#### AI 활용의 한계

1. **세밀한 표현 부족** – 표정, 동작 같은 디테일은 원하는 대로 나오기까지 반복 조정 필요.
2. **일관성 유지 어려움** – 컷마다 캐릭터가 조금씩 다르게 표현되는 경우 있음.
3. **프롬프트 의존성** – 원하는 결과를 얻으려면 문구를 상당히 구체적으로 작성해야 함.



#### 다음 참여자에게 드리는 팁

* 컷별 스토리와 대사를 **먼저 텍스트로 확정**하고 AI 제작에 들어가면 시행착오가 줄어든다.
* 프롬프트는 **구체적이고, 반복 수정할 각오**가 필요하다.


<img width="1024" height="1536" alt="ChatGPT Image 2025년 7월 31일 오후 09_45_45" src="https://github.com/user-attachments/assets/b9592e7c-8141-4d85-9c01-9016df87b963" />

<img width="1440" height="900" alt="스크린샷 2025-08-01 오후 1 05 49" src="https://github.com/user-attachments/assets/95f4a3c1-7bea-4550-822a-245dd7625e6b" />
